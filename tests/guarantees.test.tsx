...
describe("guarantee: nothing irreversible without explicit consent", () => {
  ...
  it("diff changes never apply until every hunk is explicitly decided", () => {
    const onApply = vi.fn();
    render(
      <DiffReviewCard
        title="Edit copy"
        ...
        onApply={onApply}
        hunks={[
          { id: "a", label: "Headline", before: ["old"], after: ["new"] },
          { id: "b", label: "CTA", before: ["go!"], after: ["Start free"] },
        ]}
      />,
    );
    ...
    expect((apply as HTMLButtonElement).disabled).toBe(true); // one hunk still pending
    fireEvent.click(screen.getAllByRole("button", { name: "Reject" })[1]);
    expect((apply as HTMLButtonElement).disabled).toBe(false);
    fireEvent.click(apply);
    expect(onApply).toHaveBeenCalledOnce();
    // the receipt records the rejection, not just the acceptance
    expect(screen.getByText(/1 applied · 1 rejected/)).toBeTruthy();
  });
});
